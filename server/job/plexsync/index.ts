          ...
          if (media) {
            ...
            [true, false].forEach(async (is4k) => {
              if (
                ...
                const availableRequest = requests.find((request) =>
                ...
                if (
                  availableRequest &&
                  availableRequest.status !== MediaRequestStatus.AVAILABLE
                ) {
                  availableRequest.status = MediaRequestStatus.AVAILABLE;
                  await requestRepository.save(availableRequest);
                } else {
